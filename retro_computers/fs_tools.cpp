...
AutoCloseFile::AutoCloseFile(FILE *file) { file_ = file; }
...
AutoCloseFile::~AutoCloseFile() { Close(); }
...
void LoadFile(const std::string &file_name, std::function<void *(size_t)> allocate) {
  ...
  if (result0 != 0) {
    throw std::runtime_error(std::string("Can't check file size ") + std::string(file_name) + std::string(", errno ") +
                             std::to_string(errno));
  }

  if (buff.st_size > SIZE_MAX) {
    throw std::runtime_error(std::string("Too big file ") + std::string(file_name) + std::string(", current size ") +
                             std::to_string(buff.st_size) + std::string(", max size ") + std::to_string(SIZE_MAX));
  }
  ...
  if ((buffer != nullptr) && (buffer_size > 0U)) {
    ...
    if (result2 != buffer_size) {
      throw std::runtime_error(std::string("Can't read file ") + std::string(file_name) + std::string(", errno ") +
                               std::to_string(errno) + std::string(", result ") + std::to_string(result2) +
                               std::string(", bufferSize ") + std::to_string(buffer_size));
    }
  }
}
...
void LoadFile(const std::string &file_name, size_t max_file_size, std::vector<uint8_t> *out) {
  ...
  FsTools::LoadFile(file_name, [out, max_file_size](size_t size) {
    if (size > max_file_size) {
      throw std::runtime_error(std::string("File ") + std::string(" longer then ") + std::to_string(max_file_size) +
                               std::string(" bytes"));
    }
    out->resize(size);
    return out->data();
...
void SaveFile(const std::string &file_name, const void *data, size_t size) {
  ...
  if (size != 0U) {
    ...
    if (result0 != size) {
      ...
      throw std::runtime_error(std::string("Can't write file ") + std::string(file_name) + std::string(", errno ") +
                               std::to_string(errno));
    }
  }
}
...
dirent64 *FindFiles::Item() { return item_; }
